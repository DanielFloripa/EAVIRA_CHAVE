...
#Configs for new instance
    ...
    ROOT=`pwd`
    echo "sucesso" | sudo -S apt-get update
    echo "sucesso" | sudo -S apt-get upgrade -y
    echo "sucesso" | sudo -S apt-get install git vim nano htop python python-numpy python-pip
    echo "sucesso" | sudo -S pip install -r ${ROOT}/misc/requirements.txt    #    This project:
    #git clone https://github.com/DanielFloripa/CHAVE-Sim.git
    #	Parallel project: original #(wget -O - pi.dk/3 || curl pi.dk/3/ || fetch -o - http://pi.dk/3) | bash -
    PARALLEL=parallel
    command -v ${PARALLEL} >/dev/null 2>&1 || {
        bash ${ROOT}/misc/parallel/3.sh
