...
echo -e "\tRunning globally since: " ${CS_START}
...
if [ ${CS_isAWS} == true ]; then
    for TEST in ${TEST_LIST[@]}; do
        if [ "$TEST" == "EUCA" ]; then
            parallel --bar -j16 ${CS_PY} __init__.py -nit ${NIT[@]} -in ${SRC_LIST[@]} -az ${AZ_CONF[@]} -ha ${SRC_LIST_PLUS[@]} -alg "EUCA" -wt {1} ::: $(seq ${WT[0]} ${WT[1]} ${WT[2]}) :::
        else
            parallel --bar -j16 ${CS_PY} __init__.py -nit ${NIT[@]} -in ${SRC_LIST[@]} -az ${AZ_CONF[@]} -ha ${SRC_LIST_PLUS[@]} -alg ${TEST} -ca {1} -ff {2}  -wt {3} -lock {4} -ovc {5} -cons {6} -repl {7} ::: \
                ${CONSOLID_ALGO[@]} ::: \
                ${FF_LIST[@]} ::: \
                $(seq ${WT[0]} ${WT[1]} ${WT[2]}) ::: \
                ${LOCK_CASE[@]} ::: \
