    ...
    def run(self):
        ...
        while self.global_time <= self.api.demand.max_timestamp:
            for az in self.api.get_az_list():
                ...
                _, on, _ = az.get_hosts_density(just_on=True)
                this_metric = {'step': self.global_time,
                               'az_id': az.az_id,
                               ...
                               'objective': objective,
                               'energy': None}
                az.take_snapshot_for(['energy_acum_l', 'placement_l'], this_metric)

            self.global_time += self.window_time
            if self.global_time % 10000 == 0:
                elapsed = time.time() - start
                self.logger.critical("gt: {} , time:{} , it toke: {:.3f}s. left {}".format(
                    self.global_time, time.strftime("%H:%M:%S"), elapsed, (self.api.demand.max_timestamp - self.global_time)))
                start = time.time()
                ...
                if this_state == "START":
                    ...
                    if az.allocate_on_host(vm, vm.host_id):
                        ...
                        if len(new_host_on) > self.sla.metrics.get(az.az_id, 'max_host_on_i'):
                            ...
                            # self.logger.info("{}\t New max host in use: {} at ts: {} s gt: {}".format(
