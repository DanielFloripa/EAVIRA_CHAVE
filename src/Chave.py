...
class Chave(object):
    ...
    def __repr__(self):
        return repr([self.logger, self.nit, self.trigger_to_migrate, self.frag_percentual, self.pm_mode, self.ff_mode,
                     self.window_size, self.window_time, self.has_overbooking, self.all_vms_dict, self.all_op_dict,
                     self.all_ha_dict, self.sla])

    def run(self):
        for az in self.api.localcontroller_list[0].az_list:
            print az.az_id
        #self.consolida()
        pass
    ...
    def chave_consolida(self, dc):
        ...
        while arrival_time < this_cycle and len(op_dict_temp.items()) > 0:
            ...
            for op_id, op_vm in op_dict_temp.items():
                ...
                vm = self._opdict_to_vmlist(op_vm.get_id(), dc.vm_list)
            ...
            for eachHost in new_dc.get_host_list():
                if new_dc.allocate_on_host(vm):
                    ...
                    self.logger.error("PROBLEM ON MIGRATE after overbooking"+
                                      str(vm.get_id())+str(eachHost.get_id())+str(eachHost.overb_count))
        return new_dc
