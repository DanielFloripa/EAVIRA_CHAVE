...
install.packages(c("RSQLite", "ggplot2","reshape2", "plyr", "fmsb"),repos = "http://cran.us.r-project.org")
library("fmsb")
library("plyr")
library("reshape2")
library("ggplot2")
...
date = "18.06.30-22.24.17/" #"18.06.27-12.42.39/"
...
q_az_load_val_0 <- 'SELECT round(val_0, 5) FROM az_load_l'
q_az_load_val_0_gvt <- 'SELECT round(val_0, 5), gvt FROM az_load_l'
...
fun_histogram_each_az <- function(con_az, i){
    ...
    BREAKS<-20
    pdf_name1 <- paste0(toString("02-Hist_AZ"), toString(i), toString(".pdf"))
    p3 <- ggplot(dat, aes(dat$AZLoad))+
        geom_histogram(bins=BREAKS)+
        labs(title=paste0("Load Histogram AZ",toString(i)),
             x ="Load", 
             y = "Frequency")
    ggsave(pdf_name1, p3, width = 4, height = 4, scale = 1)
    # SCATTER:
    #plot_name <- paste0(toString("03-Scatter_AZ"), toString(i))
    #pdf_name <- paste0(toString(plot_name), toString(".pdf"))
    #plot(gvt, AZLoad ,  pch=1 , bg="white" , cex=1 , col=rgb(0.1,0.3,0.5,0.5), ylab="AZ_Load" , xlab="Time" )
    #ggplot(dat, aes(x=gvt, y=AZLoad)) +
    #    geom_point(shape=1) +    # Use hollow circles
    #    geom_smooth(method=lm)#+ggtitle(plot_name)+xlab("Time") + ylab("AZLoad")
    #ggsave(pdf_name)
}
i<-1
for (az_con in az_con_list){
...
#pdf_nameCDF <- paste0(toString("03-CDF_AZs"), toString(".pdf"))
...
df_cdf1 <- ddply(df_cdf1, (.variable), transform, ecd=ecdf(value)(value))
df_cdf2 <- data.frame(AZ2 = x2$val_0)
df_cdf2 <- melt(df_cdf2)
df_cdf2 <- ddply(df_cdf2, (.variable), transform, ecd=ecdf(value)(value))
...
########### 2.2.1) FUNCIONA MAS SEM LEGENDAS: #########
#d <- data.frame(x = c(0,1))
#ll <- Map(f  = stat_function, colour = c('red', 'darkgreen', 'blue', 'magenta', 'orange', 'black'), fun = list(cdf1, cdf2, cdf3, cdf4, cdf5, cdf6), geom = 'step')
#cdf <- ggplot(data = d, aes(x = x), show.legend=TRUE) + ll +
#    stat_function(cdf1, color="red", geom = 'step')+
#    scale_fill_manual(values=c('red', 'darkgreen', 'blue', 'magenta', 'orange', 'black'),
#                      name="AZs", 
#                      breaks=c("cdf1", "cdf2", "cdf3", "cdf4", "cdf5", "cdf6"),
#                      labels=c("az1", "az2", "az3", "az4", "az5", "az6"),
#                      drop = FALSE
#    ) +
#    theme(legend.position="right")+
#    labs(title="CDF - Cumulative Distribution Function",
...
#         y = "CDF(x)",
#         fill="AZ_id")
#ggsave("03-CDF_All_AZs.pdf", cdf, width = 5, height = 5, scale = 0.5)
...
############################## 2.6) THEORETICAL (Numb. of AZs / Regions) ######
